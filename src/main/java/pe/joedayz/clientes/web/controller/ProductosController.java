...
public class ProductosController {
	...
	public ModelMap getAll1(@PageableDefault(size = 10) Pageable pageable, @RequestParam(name = "value", required = false) String value, Model model) {
		...
		return new ModelMap().addAttribute("clientes", productoRepository.findAll(pageable));
	
	}


	@GetMapping("/productos/form")
	public ModelMap getById(@RequestParam(value = "id", required = false)Integer id, Producto cliente, Model model) {
		...
		cliente = productoRepository.findByid(id);
		}
	...
	public String saveSocio(@Valid Producto cliente, BindingResult result, SessionStatus status) {
