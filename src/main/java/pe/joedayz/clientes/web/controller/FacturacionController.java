...
public class FacturacionController {
	...
	public ModelMap getAll1(@PageableDefault(size = 10) Pageable pageable, @RequestParam(name = "value", required = false) String value, Model model) {
		...
		String url="http://localhost:8080/veronica/oauth/token?username=user&password=veronica&grant_type=password";
		HttpHeaders headers = new HttpHeaders();
		headers.add("Authorization", "Basic " + Base64.getUrlEncoder().encodeToString("veronica:veronica".getBytes(Charset.forName("UTF-8"))));
		HttpEntity myRequest = new HttpEntity(headers);
		RestTemplate plantilla = new RestTemplate();
		
		token= plantilla.postForObject(url,myRequest,Token.class);
	
		model.addAttribute("tok", token);
	...
	public ModelMap getById(@RequestParam(value = "id", required = false)Integer id, Usuario cliente, Model model) {
	...
	@Transactional
	@PostMapping("/facturas/form")
	public String saveSocio(@Valid Usuario cliente, BindingResult result, SessionStatus status) {
		...
		cliente.setGrupoMenus("1,2,3,4,6,7,8");
